...

        {
            ...
            if (grammar_triggers != data.end())
            {
                for (const auto &t : *grammar_triggers)
                {
                    ...
                    auto ids =
                        common_tokenize(vocab, trigger.word, /* add_special= */ false, /* parse_special= */ true);
                    if (ids.size() == 1)
                    {
            ...
            {
                GGML_ASSERT(params.sampling.grammar_trigger_tokens.size() > 0 ||
                            params.sampling.grammar_trigger_words.size() > 0);
            }
        }
...
struct server_context
{
    ...
    bool launch_slot_with_task(server_slot &slot, const server_task &task)
    {
        ...
        if (slot.n_predict > 0 && slot.params.n_predict > slot.n_predict)
        {
            ...
            SLT_WRN(slot, "n_predict = %d exceeds server configuration, setting to %d", slot.params.n_predict,
                    slot.n_predict);
        ...
        for (int32_t i = 0; i < batch.n_tokens; i += n_batch)
        {
            ...
            if (ret != 0)
            {
                if (n_batch == 1 || ret < 0)
                {
                    ...
                    SRV_ERR(
                        "failed to decode the batch: KV cache is full - try increasing it via the context size, i = "
                        "%d, n_batch = %d, ret = %d\n",
                        i, n_batch, ret);
                    for (auto &slot : slots)
                    {
                ...
                SRV_WRN(
                    "failed to find free space in the KV cache, retrying with smaller batch size - try increasing it "
                    "via the context size or enable defragmentation, i = %d, n_batch = %d, ret = %d\n",
                    i, n_batch, ret);
...
int main(int argc, char **argv)
{
    ...
    const auto handle_slots = [&](const httplib::Request &req, httplib::Response &res)
    {
        ...
        {
            res_error(res, format_error_response("This server does not support slots endpoint. Start it with `--slots`",
    ...
    const auto handle_slots_save = [&ctx_server, &res_error, &res_ok, &params](const httplib::Request &req,
    ...
    const auto handle_slots_restore = [&ctx_server, &res_error, &res_ok, &params](
                                          const httplib::Request &req, httplib::Response &res, int id_slot)
    {
        json request_data = json::parse(req.body);
        std::string filename = request_data.at("filename");
        if (!fs_validate_filename(filename))
    ...
    const auto handle_slots_erase = [&ctx_server, &res_error, &res_ok](const httplib::Request & /* req */,
                                                                       httplib::Response &res, int id_slot)
    {
    ...
    const auto handle_slots_action = [&params, &res_error, &handle_slots_save, &handle_slots_restore,
                                      &handle_slots_erase](const httplib::Request &req, httplib::Response &res)
    {
    ...
    const auto handle_props = [&ctx_server, &res_ok](const httplib::Request &, httplib::Response &res)
    {
        ...
        json data = {
            ...
            {"bos_token",
             common_token_to_piece(ctx_server.ctx, llama_vocab_bos(ctx_server.vocab), /* special= */ true)},
            {"eos_token",
             common_token_to_piece(ctx_server.ctx, llama_vocab_eos(ctx_server.vocab), /* special= */ true)},
    ...
    const auto handle_props_change = [&ctx_server, &res_error, &res_ok](const httplib::Request &req,
                                                                        httplib::Response &res)
    {
    ...
    const auto handle_answer = [&handle_answer_impl](const httplib::Request &req, httplib::Response &res)
    {
        ...
        return handle_answer_impl(SERVER_TASK_TYPE_COMPLETION, data, req.is_connection_closed, res,
                                  OAICOMPAT_TYPE_NONE, req);
    };

    const auto handle_chat_answer = [&ctx_server, &params, &res_error, &handle_answer_impl](
                                        const httplib::Request &req, httplib::Response &res)
    {
        ...
        return handle_answer_impl(SERVER_TASK_TYPE_COMPLETION, data, req.is_connection_closed, res,
                                  OAICOMPAT_TYPE_CHAT, req);
    };

    // same with handle_chat_answer, but without inference part
    const auto handle_apply_template = [&ctx_server, &params, &res_ok](const httplib::Request &req,
                                                                       httplib::Response &res)
    {
        auto body = json::parse(req.body);
        json data = oaicompat_completion_params_parse(body, params.use_jinja, params.reasoning_format,
    ...
    const auto handle_embeddings_impl = [&ctx_server, &res_error, &res_ok](const httplib::Request &req,
                                                                           httplib::Response &res,
                                                                           oaicompat_type oaicompat)
    {
        const json body = json::parse(req.body);
        ...
        {
            res_error(res,
                      format_error_response("\"input\" or \"content\" must be provided", ERROR_TYPE_INVALID_REQUEST));
            return;
        }
        ...
        // write JSON response
        json root = oaicompat == OAICOMPAT_TYPE_EMBEDDING ? format_embeddings_response_oaicompat(body, responses, use_base64) : json(responses);
        res_ok(res, root);
    };
    ...
    const auto handle_embeddings_oai = [&handle_embeddings_impl](const httplib::Request &req,
                                                                 httplib::Response &res)
    {
        handle_embeddings_impl(req, res, OAICOMPAT_TYPE_EMBEDDING);
    };

    const auto handle_rerank = [&ctx_server, &res_error, &res_ok](const httplib::Request &req,
                                                                  httplib::Response &res)
    {
        ...
        if (documents.empty())
        {
            res_error(res, format_error_response("\"documents\" must be a non-empty string array",
                                                 ERROR_TYPE_INVALID_REQUEST));
            return;
        }

        llama_tokens tokenized_query =
            tokenize_input_prompts(ctx_server.vocab, query, /* add_special */ false, true)[0];
        ...
        {
            std::vector<server_task> tasks;
            std::vector<llama_tokens> tokenized_docs =
                tokenize_input_prompts(ctx_server.vocab, documents, /* add_special */ false, true);
    ...
    ctx_server.queue_tasks.on_new_task(
        [&ctx_server](const server_task &task)
